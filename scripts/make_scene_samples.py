...
def sample(
    ...
    if not recompute_conditional:
        ...
        output_indices = model.sample(
            z_indices, c_indices, steps=x_max * y_max, temperature=temperature, sample=True, top_k=top_k
        )
    else:
        output_indices = torch.zeros((no_samples, y_max, x_max), device=device).long()
        for predict_y, predict_x in tqdm(
            product(range(y_max), range(x_max)), desc="sampling_image", total=x_max * y_max
        ):
            ...
            new_index = model.sample(
                z_indices, c_indices, steps=1, temperature=temperature, sample=True, top_k=top_k
            )
...
if __name__ == "__main__":
    ...
    for batch_no, batch in p_bar_1:
        ...
        for i, annotations in tqdm(
            enumerate(batch["annotations"]), desc="within_batch", total=data_loader.batch_size
        ):
            imgs = sample(
                model,
                annotations,
