...
def load_annotations(
    ...
    category_no_for_id: Dict[str, int],
    ...
    with open(descriptor_path) as file:
        ...
        if "train" in str(descriptor_path) and i < 14000000:
            warnings.warn(
                f"Running with subset of Open Images. Train dataset has length [{len(annotations)}]."
            )
...
class AnnotatedObjectsOpenImages(AnnotatedObjectsDataset):
    def __init__(self, use_additional_parameters: bool, **kwargs):
        ...
        annotations = load_annotations(
            self.paths["annotations"], self.min_object_area, self.category_mapping, self.category_number
        )
        self.annotations = self.filter_object_number(
            annotations, self.min_object_area, self.min_objects_per_image, self.max_objects_per_image
        )
